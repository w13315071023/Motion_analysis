...
//构造函数
lincoln_class::CMySerialPort::CMySerialPort()
{
...
//打开串口
BOOL lincoln_class::CMySerialPort::Open(int nPort, int nBaud)
{
	if (m_bOpened)
		return TRUE;
...
//关闭串口
BOOL lincoln_class::CMySerialPort::Close(void)
{
...
//向串口写入数据，类内部使用
BOOL lincoln_class::CMySerialPort::WriteCommByte(unsigned char ucByte)
{
	BOOL bWriteStat;
...
//向串口发送数据
int lincoln_class::CMySerialPort::SendData(const char *buffer, int size)
{
...
int lincoln_class::CMySerialPort::ReadDataWaiting(void)
{
	if (!m_bOpened || m_hIDComDev == NULL) return (0);
...
//读取来自串口的数据
int lincoln_class::CMySerialPort::ReadData(void *buffer, int limit)
{
